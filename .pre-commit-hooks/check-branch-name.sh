...
# colon and capitalises the type, so it can never satisfy cz check.
...
if ! printf '%s' "$branch" | grep -Eq "^(${TYPES})/[a-z0-9][a-z0-9._-]*$"; then
  ...
  it is what the pull request title is derived from.

  Rename it with:

      git branch -m ${branch} docs/some-slug
...
if [ "$ahead" -gt 1 ]; then
  ...
  request title to the branch name.
