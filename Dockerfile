...
# uv comes from its own published image so the version here and the one
# developers run locally stay in step.
COPY --from=ghcr.io/astral-sh/uv:0.11.10 /uv /uvx /usr/local/bin/

# git and make for the workflow itself; nodejs so the pre-commit hooks written
# in those languages (markdown-link-check) do not have to bootstrap a toolchain
